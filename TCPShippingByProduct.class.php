...
class TCPShippingByProduct {
	...
	function tcp_product_metabox_custom_fields_after_price( $post_id ) { ?>
		<tr valign="top">
			<th scope="row"><label for="tcp_shipping"><?php _e( 'Shipping', 'tcp' );?>:</label></th>
			<td><input type="text" min="0" placeholder="<?php tcp_get_number_format_example(); ?>" name="tcp_shipping" id="tcp_shipping" value="<?php echo tcp_number_format( tcp_get_the_shipping_by_product( $post_id ) );?>" class="regular-text" style="width:12em" />&nbsp;<?php tcp_the_currency();?></td>
		</tr><?php
	}
	...
	function tcp_buy_button_column_titles( $titles, $post_id ) {
		array_splice( $titles, 1, 0, __( 'Shipping', 'tcp') );
		return $titles;
	}
	...
	function tcp_shipping_flat_rate_calculate_by_methods( $calculate_by_methods ) {
		$calculate_by_methods['product'] = __( 'Product', 'tcp' );
